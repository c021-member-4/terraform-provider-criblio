...
func providerRestBaseURL(serverURL string, credentials *auth.CriblConfig) string {
...
func providerRestCredentials(clientOauth *providerOAuthConfig, params map[string]string, explicitParams map[string]bool) *auth.CriblConfig {
	...
	if clientOauth.ClientID != "" {
		credentials.ClientID = clientOauth.ClientID
	}
	if clientOauth.ClientSecret != "" {
	...
	if value := os.Getenv("CRIBL_ONPREM_USERNAME"); value != "" {
		credentials.OnpremUsername = value
	}
	if value := os.Getenv("CRIBL_ONPREM_PASSWORD"); value != "" {
		credentials.OnpremPassword = value
	}
...
func (p *CriblioProvider) DataSources(ctx context.Context) []func() datasource.DataSource {
	return []func() datasource.DataSource{
		...
		NewGrokDataSource,
		NewGroupDataSource,
		NewGroupSystemSettingsDataSource,
		...
		NewProjectPipelineDataSource,
		NewProjectPipelinesDataSource,
		NewRegexDataSource,
		NewRoutesDataSource,
		NewSchemaDataSource,
		...
		NewSearchSourceDataSource,
		NewSearchSourcesDataSource,
		NewSecretDataSource,
		NewSourceDataSource,
